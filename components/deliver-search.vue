        ...
        <div class="deliver-recode-box flex">
          <div class="recode-img">
            <el-image
              ...
              fit="cover"
              :src="deliverOrder.goods_image"
            />
            <div class="float-count flex row-center">
              {{ $t("共") }}{{ deliverOrder.goods_count }}{{ $t("件商品") }}
            </div>
          </div>
          <div class="recode-info-container m-l-10">
            <div class="flex">
              <div class="recode-label">{{ $t("物流状态：") }}</div>
              <div class="primary lg" style="font-weight: 500">
                {{ delivery.title }}
              </div>
            </div>
            <div class="flex" style="margin: 6px 0">
              <div class="recode-label">{{ $t("快递公司：") }}</div>
              <div>{{ deliverOrder.express_name || "-" }}</div>
            </div>
            <div class="flex">
              <div class="recode-label">{{ $t("快递单号：") }}</div>
              <div>{{ deliverOrder.invoice_no || "-" }}</div>
              <div class="copy-btn primary flex row-center" @click="onCopy">
                {{ $t("复制") }}
              </div>
            </div>
          </div>
